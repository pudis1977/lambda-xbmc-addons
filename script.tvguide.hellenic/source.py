...
class Database(object):
        ...
        while True:
            ...
            try:
                ...
                if cancel_requested_callback is None:
                    xbmc.log('[script.tvguide.hellenic] Database is locked, bailing out...', xbmc.LOGDEBUG)
                    break
                else:  # ignore 'database is locked'
                    xbmc.log('[script.tvguide.hellenic] Database is locked, retrying...', xbmc.LOGDEBUG)

            except sqlite3.DatabaseError:
                self.conn = None
                if self.alreadyTriedUnlinking:
                    xbmc.log('[script.tvguide.hellenic] Database is broken and unlink() failed', xbmc.LOGDEBUG)
                    break
                else:
                    try:
    ...
    def _updateChannelAndProgramListCaches(self, date, progress_callback, clearExistingProgramList):
        ...
        c = self.conn.cursor()
        try:
            xbmc.log('[script.tvguide.hellenic] Updating caches...', xbmc.LOGDEBUG)
...
class HellenicTvSource(Source):
    KEY = 'hellenictv'

    def __init__(self, addon):
        self.htvPath = xbmcaddon.Addon('plugin.video.hellenic.tv').getAddonInfo('path')
        self.xmltvFile = xbmc.translatePath(os.path.join(self.htvPath,'xmltv.xml'))
...
def instantiateSource():
    ...
    try:
        ...
        activeSource = SOURCES['Hellenic.tv']
